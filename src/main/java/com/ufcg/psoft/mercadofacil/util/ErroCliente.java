...
public class ErroCliente {
	...
	static final String CLIENTE_NAO_CADASTRADO = "Cliente com id %s não está cadastrado";
	
	static final String CLIENTES_NAO_CADASTRADOS = "Não há clientes cadastrados";
	...
	public static ResponseEntity<CustomErrorType> erroClienteNaoEncontrado(long id) {
		return new ResponseEntity<CustomErrorType>(new CustomErrorType(String.format(ErroCliente.CLIENTE_NAO_CADASTRADO, id)),
				HttpStatus.NOT_FOUND);
	}
	
	public static ResponseEntity<CustomErrorType> erroSemClientesCadastrados() {		
		return new ResponseEntity<CustomErrorType>(new CustomErrorType(ErroCliente.CLIENTES_NAO_CADASTRADOS),
